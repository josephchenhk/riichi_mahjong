...
class VisibleTable(Table):
#    def __init__(self, previous_ai=False):
#        Table.__init__(self, previous_ai=False)
        
    def _init_players(self,):
        self.player = VisiblePlayer(self, 0, self.dealer_seat, self.previous_ai)

        self.players = [self.player]
        for seat in range(1, self.count_of_players):
            # the enemy players are visible.
            player = VisiblePlayer(self, seat, self.dealer_seat, self.previous_ai)
            self.players.append(player)